...
"""
    Views da aplicação gerencia.usr
"""
...
def my_view(request):
    ...
    if user is not None:
        ...
        #Template login/senha invalido
        print 'Login ou senha invalida.'

@login_required
def logout(request):
    """
        Faz logout de usuários logados
    """
    logout(request)
...
def index(request):
    ...
    t = loader.get_template('gerencia/usr/user_index.html') #indica o template
...
def add_user(request):
    if request.POST:
        ...
        return render_to_response('gerencia/usr/adicionar_user.html',
            {'form': form.as_p(),
...
def user_details(request, id):
    ...
    return render_to_response('gerencia/usr/user_details.html', {'user': user, 'MEDIA_URL': settings.MEDIA_URL})
...
def alterar(request, id):
    ...
    return render_to_response('gerencia/usr/alterar_user.html', {'user': user, 'form': form, 'MEDIA_URL': settings.MEDIA_URL})
...
def mostra_dados_group(request, id):
    ...
    return render_to_response('gerencia/usr/group_dados.html', {'form':f.as_table(), 'id':id, 'MEDIA_URL': settings.MEDIA_URL})
...
def group_index(request):
    ...
    t = loader.get_template('gerencia/usr/group_index.html') #indica o template
    c = Context ({'lista': lista1, 'MEDIA_URL': settings.MEDIA_URL}) #cria uma variável “lista” para o template
    return HttpResponse(t.render(c)) #renderiza o template
...
def add_group(request):
    if request.POST:
        ...
        f = GroupModelForm()
        return render_to_response('gerencia/usr/adicionar_group.html', {'form':f.as_table(), 'MEDIA_URL': settings.MEDIA_URL})
...
def search(request):
    ...
    if query:
        ...
        results = []
    return render_to_response("gerencia/usr/user_busca.html", {
        "results": results,
        "query": query,
        'MEDIA_URL': settings.MEDIA_URL
    })
